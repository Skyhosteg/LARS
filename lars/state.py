...
class ReasoningStep(BaseModel):
    ...
    latest_cot: Optional[str] = Field(
        default=None,
        description="The most recent CoT produced by the executor for this step. "
        "Used by the merger to detect references the description may not capture.",
    )
...
class StateVector(BaseModel):
    ...
    active_overrides: list[str] = Field(
        ...
        description="User interrupts not yet consumed by a CoT. The "
        "executor injects these into its prompt so the LLM uses the new "
        "keywords (e.g., Twitter instead of Facebook) from the first step. "
        "Drained on each step execution.",
    )
