...
class LiveAgent:
    ...
    def run(self, goal: str, max_steps: int = 5) -> StateVector:
        ...
        for step_num in range(max_steps):
            ...
            # Drain consumed overrides (they were injected into the prompt)
            if s.active_overrides:
                s = s.model_copy(update={"active_overrides": []})

            # b. Mark completed: move from pending to completed
            # Also store the latest CoT in the step so the merger can see it.
            completed_step = next_step.model_copy(update={
                "status": "completed",
                "latest_cot": cot,
            })
            s = s.model_copy(update={
                "steps_completed": s.steps_completed + [completed_step],
            ...
            if user_text:
                ...
                # v0.5.0: Refresh pending step descriptions so future
                # merges can detect the new keywords (Facebook, Cairo, etc).
                if s.steps_pending:
                    intent_payload = intent.value or intent.new_value or user_text
                    s = self.extractor.refresh_pending(
                        s, intent.type, intent_payload
                    )
                    _cprint("STATE", s.summary(), "green")
                # v0.5.1: Track active overrides so the next CoT is
                # generated with the new keywords in mind.
                s = s.model_copy(update={
                    "active_overrides": s.active_overrides + [user_text],
                })
            else:
                s_prev = s
