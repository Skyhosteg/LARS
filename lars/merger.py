...
class StateMerger:
    ...
    def _narrow(
        ...
        v1.1: now also rewrites PENDING steps so the user's intent
        propagates to the unexecuted plan, not just the executed part.
        """
        trace = MergeTrace(
        ...
        def _maybe_rewrite(text: str) -> str:
            if broad_pattern.search(text) or re.search(
                r"\begypt\b|\ball\s+cities\b|\bnationwide\b",
                text, re.I,
            ):
                modified = re.sub(r"\begypt\b", new_value, text, flags=re.I)
                modified = re.sub(
        ...
        def _process_step(step: ReasoningStep) -> tuple[ReasoningStep, bool]:
            """Rewrite a step's description and CoT if either contains
            broad-scope references. Returns (new_step, modified_bool)."""
            rew_desc = _maybe_rewrite(step.description)
            rew_cot = _maybe_rewrite(step.latest_cot or "")
            if rew_desc != step.description or rew_cot != (step.latest_cot or ""):
                new_step = step.model_copy(update={
                    "description": rew_desc,
                    "latest_cot": rew_cot if rew_cot != (step.latest_cot or "") else step.latest_cot,
                })
                return new_step, True
            return step, False

        new_steps_done: list[ReasoningStep] = []
        new_steps_pending: list[ReasoningStep] = []

        for step in s.steps_completed:
            new_step, modified = _process_step(step)
            if modified:
                new_steps_done.append(new_step)
                trace.modified_steps.append(step.step_id)
            else:
                new_steps_done.append(step)
                trace.preserved_steps.append(step.step_id)

        for step in s.steps_pending:
            new_step, modified = _process_step(step)
            if modified:
                new_steps_pending.append(new_step)
                trace.modified_steps.append(step.step_id)
            else:
                new_steps_pending.append(step)
                trace.preserved_steps.append(step.step_id)

        new_assumptions: list[str] = []
        for i, asm in enumerate(s.assumptions):
            rewritten = _maybe_rewrite(asm)
            if rewritten != asm:
                new_assumptions.append(rewritten)
        ...
        new_decisions = list(s.decisions) + [Decision(
        ...
        new_sv = StateVector(
            ...
            confidence=max(0.0, s.confidence - 0.02),
            version=s.version + 1,
            raw_cot=s.raw_cot,
        )
        return new_sv, trace
    ...
    def _replace(
        ...
        v1.1: now also processes pending steps."""
        trace = MergeTrace(
            intent_type=intent.type,
            alpha=self.alpha, beta=self.beta, gamma=self.gamma,
        )
        ...
        new_steps_done: list[ReasoningStep] = []
        for step in s.steps_completed:
            swapped_desc = swap(step.description)
            swapped_cot = swap(step.latest_cot or "")
            if swapped_desc != step.description or swapped_cot != (step.latest_cot or ""):
                new_step = step.model_copy(update={
                    "description": swapped_desc,
                    "latest_cot": swapped_cot if swapped_cot != (step.latest_cot or "") else step.latest_cot,
                })
                new_steps_done.append(new_step)
                trace.modified_steps.append(step.step_id)
            else:
    ...
    def _remove(
        ...
        v0.5: now also checks latest_cot."""
        trace = MergeTrace(
            intent_type=intent.type,
            alpha=self.alpha, beta=self.beta, gamma=self.gamma,
        )
        ...
        for step in s.steps_completed:
            matches = False
            if value:
                if re.search(rf"\b{re.escape(value)}\b", step.description, re.I):
                    matches = True
                elif step.latest_cot and re.search(rf"\b{re.escape(value)}\b", step.latest_cot, re.I):
                    matches = True
            if matches:
