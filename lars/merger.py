...
class StateMerger:
    ...
    def _narrow(
        ...
        for step in s.steps_completed:
            rewritten = _maybe_rewrite(step.description)
            if rewritten != step.description:
                new_steps_done.append(step.model_copy(update={"description": rewritten}))
        ...
        for step in s.steps_pending:
            rewritten = _maybe_rewrite(step.description)
            if rewritten != step.description:
                new_steps_pending.append(step.model_copy(update={"description": rewritten}))
                trace.modified_steps.append(step.step_id)
            else:
    ...
    def _replace(
        ...
        for step in s.steps_pending:
            swapped = swap(step.description)
            if swapped != step.description:
                new_steps_pending.append(step.model_copy(update={"description": swapped}))
                trace.modified_steps.append(step.step_id)
            else:
                new_steps_pending.append(step)
                trace.preserved_steps.append(step.step_id)
