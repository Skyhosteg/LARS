...
def test_active_overrides_injected_into_prompt():
    """
    v0.5.1: The executor must receive the active_overrides so the LLM
    uses the new keywords (e.g., Twitter instead of Facebook).
    """
    from lars.executor import MockStepExecutor
    from lars.state import StateVector, ReasoningStep
    ...
    s = StateVector(
        ...
        steps_pending=[
            ReasoningStep(step_id=2, description="Step 2", dependencies=[]),
        ],
        active_overrides=["focus on Cairo only", "use Twitter instead of Facebook"],
        assumptions=[],
        decisions=[],
        confidence=0.5,
    )
    ...
    block = exec._format_overrides(s)
    assert "focus on Cairo only" in block
    assert "use Twitter instead of Facebook" in block
    assert "USER OVERRIDES" in block
    print("✓ test_active_overrides_injected_into_prompt")


def test_active_overrides_empty_returns_empty_string():
    """v0.5.1: empty overrides should produce empty string."""
    from lars.executor import MockStepExecutor
    from lars.state import StateVector
    ...
    exec = MockStepExecutor()
    assert exec._format_overrides(s) == ""
    print("✓ test_active_overrides_empty_returns_empty_string")
...
if __name__ == "__main__":
    ...
    test_active_overrides_injected_into_prompt()
    test_active_overrides_empty_returns_empty_string()
