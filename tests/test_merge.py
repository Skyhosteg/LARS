...
def test_merger_v0_5_uses_latest_cot():
    """
    v0.5.0: merger must check latest_cot in addition to description.
    Simulates the real LLM case where description is generic but CoT has specifics.
    """
    s = StateVector(
        goal="Plan for Egypt market",
        steps_completed=[
            ReasoningStep(
                step_id=1,
                description="Analyze the market.",
                status="completed",
                latest_cot="Egypt has 110M people, 60% under 30, growing fitness app adoption in Cairo and Alexandria.",
            ),
            ReasoningStep(
                step_id=2,
                description="Define the audience.",
                status="completed",
                latest_cot="Primary: 18-30 year olds in Cairo, urban, middle income.",
            ),
        ],
        steps_pending=[
            ReasoningStep(step_id=3, description="Choose channels.", latest_cot=None),
            ReasoningStep(step_id=4, description="Allocate budget.", latest_cot=None),
        ],
        assumptions=[],
        decisions=[],
        confidence=0.5,
    ...
    m = StateMerger()
    intent = DeltaUParserMock().parse("focus on Cairo only")
    s_new, trace = m.merge(s, intent)

    # Step 1 should be modified (CoT had "Egypt" + "Cairo")
    step1_new = next(x for x in s_new.steps_completed if x.step_id == 1)
    assert "Cairo" in step1_new.latest_cot, f"CoT should be updated: {step1_new.latest_cot}"
    assert step1_new.step_id in trace.modified_steps, "Step 1 should be modified via CoT"

    # Step 2 should be modified (CoT had "Cairo")
    step2_new = next(x for x in s_new.steps_completed if x.step_id == 2)
    assert "Cairo" in step2_new.latest_cot

    print(f"✓ test_merger_v0_5_uses_latest_cot  (modified via CoT: {trace.modified_steps})")


def test_merger_v0_5_replace_via_cot():
    """v0.5.0: REPLACE must find the value in CoT even if description is generic."""
    s = StateVector(
        goal="Marketing plan",
        steps_completed=[
            ReasoningStep(
                ...
                status="completed",
                latest_cot="We will use Facebook ads, Instagram, and TikTok for our campaign.",
            ),
        ],
        steps_pending=[],
        assumptions=[],
    ...
    m = StateMerger()
    intent = DeltaUParserMock().parse("use Twitter instead of Facebook")
    s_new, trace = m.merge(s, intent)

    step1_new = s_new.steps_completed[0]
    assert "Twitter" in step1_new.latest_cot, f"CoT should have Twitter: {step1_new.latest_cot}"
    assert "Facebook" not in step1_new.latest_cot
    assert step1_new.step_id in trace.modified_steps
    print(f"✓ test_merger_v0_5_replace_via_cot  (CoT: '{step1_new.latest_cot[:60]}...')")
...
if __name__ == "__main__":
    ...
    test_merger_v0_5_uses_latest_cot()
    test_merger_v0_5_replace_via_cot()
